...
import { massTimeSchema } from "@/lib/validation/schedules";
...
export async function POST(request: Request) {
  ...
  const parsed = massTimeSchema.safeParse(body);
  if (!parsed.success) {
    return NextResponse.json(
...
export async function DELETE(request: Request) {
  ...
  const supabase = await createServerSupabaseClient();
  const { error } = await supabase.from("mass_times").delete().eq("id", id);
  if (error) {
    return NextResponse.json(
