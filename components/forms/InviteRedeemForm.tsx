...
export function InviteRedeemForm({ token }: { token: string }) {
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setStatus("error");
      setMessage(
        error instanceof Error ? error.message : t("redeemErrorGeneric")
      );
    }
  }
  ...
  return (
    <form
      className="mt-8 space-y-4 rounded-lg bg-surface-container-low p-6"
    ...
    >
      ...
      <input
        ...
        placeholder={t("redeemNamePlaceholder")}
        value={displayName}
      ...
      <input
        className="w-full rounded-lg border-outline-variant bg-surface-container-lowest"
        type="password"
        placeholder={t("redeemPasswordPlaceholder")}
        value={password}
      ...
      >
        {status === "saving" ? t("redeemActivating") : t("redeemActivate")}
      </button>
    </form>
  );
}
